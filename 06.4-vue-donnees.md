...
Stratégie RGPD `User` : `email` stocké en clair (clé fonctionnelle de connexion) mais soumis à anonymisation après suppression du compte (remplacement par `deleted-{uuid}@anon.supevents`). `password_hash` : Argon2id (mémoire 64 Mo, parallélisme 2, itérations 3). `first_name` / `last_name` : conservation 36 mois après le dernier ticket émis, puis purge. Aucune information n'est exposée dans les payloads d'événements asynchrones.
...
Stratégie RGPD `Payment` : aucun PAN, aucun CVC, aucune date d'expiration ne sont stockés. Conformité PCI-DSS déléguée à Stripe via Stripe Elements + 3DS2. Seule la référence `stripe_payment_intent_id` est conservée ; chiffrement applicatif AES-256-GCM au repos via la clé KMS dédiée au domaine paiement, conservation 10 ans (obligation comptable).
...
Sont documentés ici uniquement les index résultant d'une décision d'architecture, justifiés par un cas d'usage volumique ou un SLA de latence du CDC. Les index implicites des PK et FK ne sont pas listés.

| Entité      | Index                                                            | Justification                                                                                  |
|-------------|------------------------------------------------------------------|------------------------------------------------------------------------------------------------|
| `Event`     | `idx_event_published_starts_at (status, starts_at) WHERE status = 'published'` | Liste paginée du catalogue trié par date — requête la plus chaude (cible p95 < 150 ms)         |
